...
use crate::{
    models::Message,
    permissions::{
        CREATE_NEW_POSTS, CREATE_NEW_THREADS, EDIT_POSTS, EDIT_THREADS, MANAGE_COMMUNITY, MANAGE_DETAILS, MANAGE_FORUMS, MANAGE_POSTS, MANAGE_USERGROUPS, MANAGE_USERS, ROOT_ACCESS
    },
};
...
pub async fn community_management_middleware(
    ...
    let user_token = user_token_opt.unwrap();
    match get_user_from_token(user_token) {
        Some(user) => match usergroups_match_permission(user.clone().usergroups, MANAGE_COMMUNITY)
        {
            true => {
                req.extensions_mut().insert(user);
                let res = next.call(req).await?;
